...
const {getBuffer, getGroupAdmins, getRandom, banner, start, success } = require('./lib/functions');
...
//_LISTA DE CRS
const cr = "Lore-BOT v1"
const crtt = "Joega de nuevo By: Beimar"
...
async function starts() {
	const Lore = new WAConnection()
	Lore.logger.level = 'warn'
	console.log(banner.string)
	Lore.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar Lore-BOT!'))
	})

	fs.existsSync('./tiringa.json') && Lore.loadAuthInfo('./tiringa.json')
	Lore.on('connecting', () => {
		start('2', 'Conectando o Lore-BOT...')
	})
	Lore.on('open', () => {
		success('2', 'Lore-BOT conectado!!!')
	})
	await Lore.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./tiringa.json', JSON.stringify(Lore.base64EncodedAuthInfo(), null, '\t'))

//_FUCTION DE BOAS-VINDAS
Lore.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await Lore.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await Lore.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `Hola @${num.split('@')[0]}\nse bienvenido(a) al grupo: ${mdata.subject}`
				let buff = await getBuffer(ppimg)
				Lore.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await Lore.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
      ...
      Lore.on('chat-update',async (mek) => {
		try {
			...
			const { text, extendedText, image, video, sticker, document, audio } = MessageType
            ...
            const totalchat = await Lore.chats.all()
			const botNumber = Lore.user.jid
			const ownerNumber = [`59160703694@s.whatsapp.net`]
			const ownerNumberB = []
			//const ban = [`59160703694@s.whatsapp.net`]
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await Lore.groupMetadata(from) : ''
			...
			let pushname = Lore.contacts[sender] != undefined ? Lore.contacts[sender].vname || Lore.contacts[sender].notify: undefined
           ...
           mess = {
			...
			const reply = (tej) => {
				Lore.sendMessage(from, tej, text, {quoted:mek})
			}
			const mentions = (ops, memberr, sender, id) => {
				(id == null || id == undefined || id == false) ? Lore.sendMessage(from, ops.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : Lore.sendMessage(from, ops.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			const costum = (pesan, tipe, target, target2) => {
                 Lore.sendMessage(from, pesan, tipe, {quoted: {key: {fromMe: false, participant: `${target}`, ...(from ? {remoteJid: from}: {})}, message: {conversation: `${target2}` }}})
            }
...
Lore.sendMessage(from, lvup, text, {quoted: mek})
//Lore.sendMessage(from, lvup, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}}
                }
            } catch (err) {
                console.error(err)
            }
...
////FINALIZAR TTT AUTOMATICAMENTE By: Resen
			if (tttset.tttstatus == "off" && tttset.autoEndTime == "on") {
				...
				if (isLevelingOn) {
					...
					Lore.sendMessage(tttset.local,`❌ El tiempo de juego expiro ❌\n\n➣  Castigo: ${randomEndTTTXP} XP 🔮`, text, {quoted: tttset.mentionPlayer})
				} else {
					Lore.sendMessage(tttset.local,`❌ El tiempo de juego expiro ❌`, text, {quoted: tttset.mentionPlayer})
				}
...
//_"Es hora de mimir" By: Beimar
			...
			//const horAtual = dataAtual.getHours()
			...
			if (horaAtual == "10:10:00" && ammOff == "off" && isGroup) {
				...
				Lore.sendMessage(from, sonica, MessageType.audio, {mimetype: 'audio/mp4', ptt:true})
				Lore.sendMessage(from, sonico, sticker)
				ammOff = "on"
...
Lore.sendMessage(from, `flood @${sender.split('@')[0]}...\n\nAguarde 5 segundos antes de usar otro comando✅`, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}})
...
authorname = Lore.contacts[from] != undefined ? Lore.contacts[from].vname || Lore.contacts[from].notify : undefined	
...
//_FUCTION NOME DOS STICKERS
				...
				fs.writeFile(`./src/stickers/${name}.exif`, buffer, () => {	
...
(budy === 'BOT') ||
(budy === 'Bot')) {
buf = fs.readFileSync(`./pack/lala17.mp3`)
Lore.sendMessage(from, buf, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true})
}


if ((budy === "/pack") ||
(budy === "pack")) {
cleitu = fs.readFileSync(`./pack/R18 (1).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack1") ||
(budy === "pack1")) {
cleitu = fs.readFileSync(`./pack/R18 (2).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack2") ||
(budy === "pack2")) {
cleitu = fs.readFileSync(`./pack/R18 (4).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack3") ||
(budy === "pack3")) {
cleitu = fs.readFileSync(`./pack/R18 (5).jpg`)
Lore.sendMessage(from, cleitu, sticker, {quoted: mek})
}
if ((budy === "/pack4") ||
(budy === "pack4")) {
cleitu = fs.readFileSync(`./pack/R18 (6).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack5") ||
(budy === "pack5")) {
cleitu = fs.readFileSync(`./pack/R18 (7).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack6") ||
(budy === "pack6")) {
cleitu = fs.readFileSync(`./pack/R18 (8).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack7") ||
(budy === "pack7")) {
cleitu = fs.readFileSync(`./pack/R18 (9).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack8") ||
(budy === "pack8")) {
cleitu = fs.readFileSync(`./pack/R18 (10).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack9") ||
(budy === "pack9")) {
cleitu = fs.readFileSync(`./pack/R18 (11).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack10") ||
(budy === "pack10")) {
cleitu = fs.readFileSync(`./pack/R18 (12).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack11") ||
(budy === "pack11")) {
cleitu = fs.readFileSync(`./pack/R18 (13).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack12") ||
(budy === "pack12")) {
cleitu = fs.readFileSync(`./pack/R18 (14).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack13") ||
(budy === "pack13")) {
cleitu = fs.readFileSync(`./pack/R18 (15).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack14") ||
(budy === "pack14")) {
cleitu = fs.readFileSync(`./pack/R18 (16).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack15") ||
(budy === "pack15")) {
cleitu = fs.readFileSync(`./pack/R18 (17).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack16") ||
(budy === "pack16")) {
cleitu = fs.readFileSync(`./pack/R18 (18).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack17") ||
(budy === "pack17")) {
cleitu = fs.readFileSync(`./pack/R18 (19).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack18") ||
(budy === "pack18")) {
cleitu = fs.readFileSync(`./pack/R18 (20).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack19") ||
(budy === "pack19")) {
cleitu = fs.readFileSync(`./pack/R18 (21).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack20") ||
(budy === "pack20")) {
cleitu = fs.readFileSync(`./pack/R18 (22).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack21") ||
(budy === "pack21")) {
cleitu = fs.readFileSync(`./pack/R18 (23).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack22") ||
(budy === "pack22")) {
cleitu = fs.readFileSync(`./pack/R18 (24).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack23") ||
(budy === "pack23")) {
cleitu = fs.readFileSync(`./pack/R18 (25).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack24") ||
(budy === "pack24")) {
cleitu = fs.readFileSync(`./pack/R18 (26).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack25") ||
(budy === "pack25")) {
cleitu = fs.readFileSync(`./pack/R18 (27).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack26") ||
(budy === "pack26")) {
cleitu = fs.readFileSync(`./pack/R18 (28).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack27") ||
(budy === "pack27")) {
cleitu = fs.readFileSync(`./pack/R18 (29).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack28") ||
(budy === "pack28")) {
cleitu = fs.readFileSync(`./pack/R18 (30).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
if ((budy === "/pack29") ||
(budy === "pack29")) {
cleitu = fs.readFileSync(`./pack/R18 (31).jpg`)
Lore.sendMessage(from, cleitu, image, {quoted: mek})
}
...
if (budy.includes("geradorcpf")){
	...
	axios.get(`http://geradorapp.com/api/v1/cpf/generate?token=40849779ec68f8351995def08ff1e2fa`).then((res) => {
		tiringa.sendMessage(from, '[🦧] ESPERE ESTA PROCESANDO...', MessageType.text)
	...
	case 'jadian':
	if (!isGroup) return reply(from, 'este comando só pode ser usado dentro de un grupo', id)
	const up = groupMembers
	const aku = up[Math.floor(Math.random() * up.length)];
	const kamu = up[Math.floor(Math.random() * up.length)];
	const sapa = `prueba... @${(/[@c.us]/g, '')} (💘) @${(/[@c.us]/g, '')} prueba`
	tiringa.sendMessage(from, sapa, {quoted: mek})
	break
	
	case 'hi':
	if (isBanned) return reply('usted esta baneado')
	break
	...
	case 'tebakgambar':  //Case by nayla
						...
						anu2 = `► *RESPUESTA* : ${anu.result.jawaban}`
						setTimeout( () => {
						ara.sendMessage(from, anu1, image,{caption: 'Contestame.... tienes 60 segundos?', quoted: mek})
						}, 1)
						setTimeout( () => {
						costum('50 SEGUNDOS', text)
						}, 10000)                                                                                                                                   
						setTimeout( () => {
						costum('40 SEGUNDOS', text)
						}, 20000)    
						setTimeout( () => {
						costum('30 SEGUNDOS', text)
						}, 30000)    
						setTimeout( () => {
						costum('20 SEGUNDOS', text)
						}, 40000)                                       
						setTimeout( () => {
						costum('10 SEGUNDOS', text)
						}, 50000)                                                                                                                                                     
						setTimeout( () => {
	...
	case 'ttp':
	msgFilter.isFiltered(from)
	if (args.length < 1) return reply(`Use de esta forma:\nComando: ${prefix}ttp Toin ganado`)
	...
	msgFilter.isFiltered(from)
	ffffg = body.slice(5)
	if (args.length == 0) return reply( 'Converse conmigo...😁 \n(evite usar acentos)')
	...
	case 'un pan':
	if (isOwner) return reply('Oi Beimar')
	else {
	reply('un mIembro comum')
	}
	break 
	...
	//_ENVIA O VCARD
	case 'beimaru':
	const sentMsg  = await tiringa.sendMessage(from, {displayname: "Beimar", vcard: vcard}, MessageType.contact)
	break
						case 'leaderboard':
					case 'lb':
	...
	yhb = `Que tierno... @${sender.split("@")[0]} le dio un abrazo apretad a @${mentioned[0].split('@')[0]}`
	...
	yhb =  `@${mentioned[0].split('@')[0]} tiene chance ${porc}% de enamorar con @${mentioned[1].split('@')[0]}`,
	...
	case 'testime':
						setTimeout( () => {
						tiringa.sendMessage(from, 'Tu tiempo acabo', text) 
						}, 10000)
						setTimeout( () => {
	...
	if (!isOwner) return reply(mess.only.ownerB)
					   npm = `npm start`
					   reply('Reiniciando en alguns segundos...')       
					   exec(npm, (err, stdout) => {
					   if(err) return tiringa.sendMessage(from, "Comando inexistente", text, {quoted: mek})
	...
	tiringa.sendMessage(from, italuft, image, {quoted: mek, caption: `☆━✪🕴  Lore߷  🕴✪━☆\n\nChat do baiano: wa.me/+59160703694`})
	break
	...
	msgFilter.isFiltered(from)
	 ...
	 `‣ Nombre del bot: ${me.name}
	  ...
	  ‣ Dono: Beimar
	  ‣ Prefix: ${prefix}
	  ‣ Bot activo por: ${temporizador(uptime)}
	...
	` 
	  ‣Nombre: ${groupName}
	  ‣Cantidad de miembros: ${groupMembers.length}
	  ‣Total de administradores: ${groupAdmins.length}
	  ‣Criador : @${owner.split('@')[0]}
	  ‣Total de miembros: ${participants.length} membros
	  ‣Descrição:
	  ${desc ? desc : ''}`
	...
	case 'imagen':
	msgFilter.isFiltered(from)
	if (args.length < 1) return reply('pon el numbre de la imagen que deseas buscar')
	tiringa.updatePresence(from, Presence.composing)
	reply(mess.wait)
	try {
	...
	tiringa.sendMessage(from, pok, image, {quoted: mek, caption: `Info sobre: ${args}`})
	} catch {
	reply(`No econtre nada sobre ${agrs}...`)
	}
	break
	...
	//_PROMOVE UM MEMBRO DO GRUPO
		  case 'promover':
						...
						if (mentioned.length > 1) {
							...
							mentions(`El usuário: @${mentioned[0].split('@')[0]} es admin ahora`, mentioned, true)
	...
	case 'registrar':
											...
											if (!q.includes('|')) return reply(`Escriba da forma correta:\nComando: ${prefix}registrar nombre|edad\nExemplo: ${prefix}registrar Beimar|18`)
										   // if (args.length < 1) return reply(`Como se registrar:\nComando: ${prefix}registrar nome|idade\nExemplo: ${prefix}registrar Italu|18`)
											...
											if(isNaN(umurUser)) return reply(`Escriba da forma correta:\nComando: ${prefix}registrar nombre|edad\nExemplo: ${prefix}registrar Beimar|18`)
											...
											captionnya = 
	`    〘 Registrado(a) exitosamente 〙
	  Código: ${serialUser}
	╔══════════════════
	╠≽️ Dia: ${date}
	╠≽️ Hora: ${hr}
	╠≽️ Nombre de usuário: @${sender.split("@")[0]}
	╠≽️ Numero de registro: ${namaUser}
	╠≽️ Edad: ${umurUser}
	╠≽️ Link wame: wa.me/${sender.split("@")[0]}
	╠≽️ Número: ${sender.split("@")[0]}
	╚══════════════════
	Pon ${prefix}menu para lista de comandos`
	...
	case 'welcome':
						...
						if (isWelkom) return reply('ya esta activada')
							...
							reply(`digite ${prefix}welcome 1 para ativar, e 0 para desativar`)
						}
							 break
					...
					case 'clone':
						if (!isGroup) return reply(mess.only.group)
						if (!isOwner) return reply(mess.only.ownerB)
						if (args.length < 1) return reply('Mencione quien quieres robar el perfl')
						...
						try {
							...
							mentions(`Robo la foto de perfil de: @${id.split('@')[0]}`, [jid], true)
						} catch (e) {
							reply('ocurrio un erro')
						}
						break
	...
	conta = `Quanto és ${number1} ${ty} ${number2}?`
	reply(conta)
	break
	...
	//INICIO DO JOGO DA VELHA ❌ ⭕ 🔲
					case 'ttt':
						...
						case 'tttrank':
							if (!isGroup) return reply(mess.only.group)
							if (tictactoe.length < 5) return reply(from, `Nesesario que 5 personas hayan jugado...`, text)
							...
							if (!isGroup) {
								...
								if (coordX != 'a1' && coordX != 'a2' && coordX != 'a3' &&
									...
									switch (args[0]) {
										case 'a1':
											if (esp.a1 != "🔲") {
												reply('OEl espacio esta ocupado\nIntente otra coordenada')
											} else {
												esp.a1 = "❌"
									...
									if (WinnerX()) {
										if (isLevelingOn) {
											...
											tiringa.sendMessage(from, `🎉🎉 VICTÓRIA DO JOGADOR 🎉🎉\n\n➣  RECOMPENSA: +${randomTTTXP} XP 🔮`, text)
										} else {
											tiringa.sendMessage(from, `🎉🎉 VICTÓRIA DO JOGADOR 🎉🎉`, text)
										}
										...
										if (isLevelingOn) {
											...
											tiringa.sendMessage(from, `🎉🎉 VICTÓRIA DO Lore-BOT 🎉🎉\n\n➣  RECOMPENSA: ${randomTTTXP} XP 🔮`, text)
										} else {
											tiringa.sendMessage(from, `🎉🎉 VICTÓRIA DO Lore-BOT 🎉🎉`, text)
										}
	...
	case 'ecchi':
	msgFilter.isFiltered(from)
	if (!isNsfw) return reply('??finciones NSFW desactivadas en este grupo🚫')
	reply(mess.waitsfw)
